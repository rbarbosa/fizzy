...
Deploy: `bin/kamal deploy -d <destination>`. The pre-build and pre-deploy hooks publish the hotcell cell's image and reboot the accessory as needed, so a deploy ships both containers.
